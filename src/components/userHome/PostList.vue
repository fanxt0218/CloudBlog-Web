    ...
    <div class="post-item" v-for="item in postList" :key="item.id" @click="handleJumpToPostDetail(item.id)">
      ...
      <div class="post-content">
        ...
        <div class="edit-wrapper">
           <span id="edit" @click="handleJumpToEditPost(item.id)">编辑</span>
        </div>
      </div>
    </div>
 ...
 * @param postId 
 */
const handleJumpToEditPost = (postId: number) => {
    console.log('跳转编辑文章页, 文章id:', postId);
    window.open(`/editPost/${postId}`, '_blank');
}

onMounted(() => {
  loadData();
  scrollContainer.value?.addEventListener('scroll', handleScroll);
  console.log('设置了页面标题', useUserInfoStore().userName);
  document.title = `${useUserInfoStore().userName} - CloudBlog`;
});
