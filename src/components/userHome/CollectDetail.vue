      ...
      <div v-for="item in collectDetail?.records || []" :key="item.postId" class="collect-item" @click="handleJumpToPostDetail(item.authorId, item.postId)">
        ...
        <div class="collect-content">
          ...
          <div class="meta-data">
            ...
            <!-- 替换为星星图标表示收藏状态 -->
            <div class="status" @click.stop="toggleCollect(item)">
              <svg
                class="star-icon"
...
<script lang="ts" setup name="CollectDetail">
...
const loadCollectDetail = async () => {
  ...
  try {
    // 构建请求参数
    const params = {
      userId: Number(localStorage.getItem('userId')), // 这里使用固定用户ID，实际应用中应该从用户登录信息中获取
